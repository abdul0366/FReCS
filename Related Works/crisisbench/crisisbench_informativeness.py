...
Original file is located at
    https://colab.research.google.com/drive/1aGsfyE8kevc3FQhiQq3B-RX85v9nXr7d
"""
...
df_train = pd.read_csv("/home/aaadfg/fr/related_work/crisisbench/consolidated_informativeness_train.csv")
df_test = pd.read_csv("/home/aaadfg/fr/related_work/crisisbench/consolidated_informativeness_test.csv")

df_train['processed_tweet'] = df_train['text'].apply(preprocess_text)
df_test['processed_tweet'] = df_test['text'].apply(preprocess_text)

# Update this dictionary based on your dataset
encoded_dict = {"not_informative": 0, "informative": 1}

df_train['event'] = df_train['class_label'].map(encoded_dict)
df_test['event'] = df_test['class_label'].map(encoded_dict)
...
with torch.no_grad():
    for batch in test_loader:
        input_ids = batch['input_ids'].to(device)
        attention_mask = batch['attention_mask'].to(device)
        labels = batch['labels'].to(device).float()  # Convert labels to float as in training
        outputs = model(input_ids, attention_mask)
        ...
        true_labels.extend(labels.cpu().tolist())  # Ensure labels are in the list format

# Print classification report
print(classification_report(true_labels, predictions, target_names=['Not Informative', 'Informative']))
